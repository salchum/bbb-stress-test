...
async function initClient(browser, logger, joinUrl, client) {
  ...
  try {
    ...
    await connectAudio(page, logger, client.microphone, client);

    await waitForRoom(page, logger);

    if (client.webcam) {
      await startWebcam(page, logger, client);
    }

    if (client.chatText !== "") {
      await sendPublicChat(page, client.chatText, logger, client);
    }
  ...
  try {
    await runWithConcurrency(clients, JOIN_CONCURRENCY, async (client) => {
      ...
      try {
        ...
        const page = await initClient(browser, logger, joinUrl, client);
